...
def utility_partials(R, x):
    ...
    # Derivative of u[0] with respect to x[0]
    partial[0] = dp0 @ R[0,:,:] @ p1
    # Derivative of u[1] with respect to x[1]
...
def episodic_game_gradient_ascent(init, rewards, nsteps, learningrate):
    ...
    logits[0, :] = init  # Initialize logits with given initial values
    ...
    for t in range(nsteps - 1):
        ...
        utilities[t, 0] = np.dot(np.dot(p0, rewards[0, :, :]), p1)
        utilities[t, 1] = np.dot(np.dot(p0, rewards[1, :, :]), p1)
        
        # Calculate partial derivatives of utilities with respect to logits for gradient ascent
        partial = utility_partials(rewards, x)
        
        # Update logits for the next step
        logits[t + 1, 0] = x[0] + learningrate * partial[0]
        logits[t + 1, 1] = x[1] + learningrate * partial[1]

    # Calculate utilities for the last set of logits
    final_p0 = sig2(logits[-1, 0])
    final_p1 = sig2(logits[-1, 1])
    utilities[-1, 0] = np.dot(np.dot(final_p0, rewards[0, :, :]), final_p1)
    utilities[-1, 1] = np.dot(np.dot(final_p0, rewards[1, :, :]), final_p1)         
...
def utility_hessian(R, x):
    ...
    p = [sig2(x[i]) for i in range(2)]  # Compute probabilities for both players
    dp = [dsig2(sig2(x[i])) for i in range(2)]  # Compute the derivative of probabilities
    ddp = [Hsig2(sig2(x[i])) for i in range(2)]  # Compute the second derivative of probabilities

    hessian = np.zeros((2, 2))
    
    # Calculate diagonal elements
    for i in range(2):
        # The second derivative term involving the direct impact of a player's own strategy on their utility
        hessian[i, i] = ddp[i][1] * np.dot(R[i, 1, :] - R[i, 0, :], p[1 - i])

    # Calculate off-diagonal elements
    # The off-diagonal elements should capture the interaction between the players' strategies
    # and how a change in one player's strategy impacts the utility gradient of the other player.
    hessian[0, 1] = dp[0][1] * np.dot(dp[1], R[0, 1, :] - R[0, 0, :])
    hessian[1, 0] = dp[1][1] * np.dot(dp[0], R[1, :, 1] - R[1, :, 0])
...
def episodic_game_corrected_ascent(init, rewards, nsteps, learningrate):
    ...
    '''
    # Initialize the logs for logits and utilities
    logits = np.zeros((nsteps, 2))
    utilities = np.zeros((nsteps, 2))
    logits[0, :] = init  # Set the initial logits

    for t in range(nsteps - 1):
        # Compute current probabilities and utilities
        current_logits = logits[t, :]
        p0 = sig2(current_logits[0])
        p1 = sig2(current_logits[1])
        utilities[t, 0] = np.dot(np.dot(p0, rewards[0, :, :]), p1)
        utilities[t, 1] = np.dot(np.dot(p0, rewards[1, :, :]), p1)

        # Calculate the gradients (partials) and the Hessian matrix
        partials = utility_partials(rewards, current_logits)
        hessian = utility_hessian(rewards, current_logits)

        # Compute the symplectic correction matrix
        correction = symplectic_correction(partials, hessian)

        # Update logits using corrected ascent
        identity_matrix = np.eye(2)
        update_matrix = identity_matrix + correction
        logits[t+1, :] = current_logits + learningrate * np.dot(update_matrix, partials)

    # Calculate utilities for the last logits
    final_p0 = sig2(logits[-1, 0])
    final_p1 = sig2(logits[-1, 1])
    utilities[-1, 0] = np.dot(np.dot(final_p0, rewards[0, :, :]), final_p1)
    utilities[-1, 1] = np.dot(np.dot(final_p0, rewards[1, :, :]), final_p1)
...
'''
sequential_strategy = np.array([[0.2, 0.2],[0.2, 0.8]])